...
SUBTASK_LIST = BASE_SUBTASKS + SHIPPING_SUBTASKS + CONTACT_SUBTASKS + \
    ...
    APPLY_COUPON_SUBTASK + FINISH2_SUBTASKS + ['Click BestBuy GoPage3'] + FINAL_SUBTASKS
...
class BestBuy(WobConfig):
  environment_id = 'bestbuy'
  ...
  def _construct_task(self, seed: int, keep_pristine: bool = False):
    ...
    if not keep_pristine:
      ...
      base_subtasks = distractors + base_failures + ['Click BestBuy GoPage1']
    ...
    with g.stage("Shipping Address"):
      ...
      g.add_many_to_one(sources=FINISH1_SUBTASKS, sink='Click BestBuy GoPage2')

    # Connect 'Click BestBuy GoPage2' ==> stage 2 (payment) subtasks.
    g.add_one_to_many(
        source='Click BestBuy GoPage2',
    ...
    g.add_many_to_one(sources=FINISH2_SUBTASKS, sink='Click BestBuy GoPage3')
