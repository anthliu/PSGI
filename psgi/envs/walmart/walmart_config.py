...
SUBTASK_LIST = BASE_SUBTASKS + SHIPPING_SUBTASKS + CONTACT_SUBTASKS + \
    FINISH1_SUBTASKS + ['Click Walmart GoPage2'] + CREDIT_SUBTASKS + \
    GIFT_SUBTASK + ['Click G_Apply'] + PAYMENT_SUBTASKS + FINISH2_SUBTASKS + \
    ['Click Walmart GoPage3'] + FINAL_SUBTASKS
...
class Walmart(WobConfig):
  environment_id = 'walmart'
  ...
  def _construct_task(self, seed: int, keep_pristine: bool = False):
    ...
    if not keep_pristine:
      ...
      base_subtasks = distractors + base_failures + ['Click Walmart GoPage1']
    ...
    with g.stage("Address"):
      g.add_one_to_many(source='Click Walmart GoPage1',
      ...
      g.add_many_to_one(sources=FINISH1_SUBTASKS, sink="Click Walmart GoPage2")

    ##### Stage 2. Fill Credit info
    g.add_one_to_many(source='Click Walmart GoPage2', sinks=PAYMENT_SUBTASKS)
    ...
    g["Click Walmart GoPage3"] = g['Click FinishPayment']
    ...
    ##### Stage 3. Last stage
    g.add_one_to_many(source='Click Walmart GoPage3', sinks=final_subtasks)
