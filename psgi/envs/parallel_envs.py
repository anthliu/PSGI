...
from typing import Optional, Any, Callable, Sequence, Text, Union
import atexit
import sys
import traceback
import cloudpickle
...
class ParallelEnvironments(dm_env.Environment):
  ...
  def task(self) -> Sequence[Any]:
    return [env.access('task') for env in self._envs]

  @property
  def num_subtasks(self) -> Sequence[int]:
    return [env.access('num_subtasks') for env in self._envs]

  @property
  def task_embedding(self) -> Sequence[dict]:
    return [env.access('task_embedding') for env in self._envs]

  @property
  def pool_to_index(self) -> Sequence[np.ndarray]:
    return [env.access('pool_to_index') for env in self._envs]

  @property
  def index_to_pool(self) -> Sequence[np.ndarray]:
    return [env.access('index_to_pool') for env in self._envs]
  ...
  def reset_task(self, task_index: Optional[int] = None):
    promises = [env.call('reset_task', task_index) for env in self._envs]
    if not self._blocking:
      promises = [promise() for promise in promises]
    return promises

  def set_task(self, tasks: Sequence[Any]) -> None:
    promises = [env.call('set_task', task) for task, env in zip(tasks, self._envs)]
    if not self._blocking:
      promises = [promise() for promise in promises]
...
class ProcessDmEnvironment(object):
  ...
  def __getattr__(self, name: Text) -> Any:
    ...
    if name.startswith('_'):
      return super(ProcessDmEnvironment, self).__getattribute__(name)
  ...
  def step(self,
           ...
           blocking: bool = True) -> Union[dm_env.TimeStep, Promise]:
  ...
  def reset(self, blocking: bool = True) -> Union[dm_env.TimeStep, Promise]:
