...
from .utils import MOVE_ACTS, AGENT, BLOCK, WATER, EMPTY, KEY, OBJ_BIAS, \
    TYPE_PICKUP, TYPE_TRANSFORM, MAX_DIST, OID_TO_IID, TimeProfiler

__PATH__ = os.path.abspath(os.path.dirname(__file__))


class Mazemap:
  def __init__(self, rank, game_name, game_config, render=False):
    ...
    self._rank = rank if rank is not None else np.random.randint(100000)
    np.random.seed(rank)
  ...
  def set_map(
      self,
      subtask_id_list,
      map_parameters: Optional[dict] = None):
    """Set the Mazemap parameters with the new parameters (e.g. one from
      the adaptation environment).
    """
    self.subtask_id_list = subtask_id_list
    self.nb_subtask = len(subtask_id_list)

    # Set task-relevant map parameters.
    self._init_object_list = map_parameters['init_object_list']
    self._init_item_map = map_parameters['init_item_map']
    self._init_obs = map_parameters['init_obs']
    self.agent_init_pos_x = map_parameters['init_pos_x']
    self.agent_init_pos_y = map_parameters['init_pos_y']
    self.distance = map_parameters['distance']
  ...
  def move_to_closest_obj(self, oid, max_step):
    ...
    for obj in self.object_list:
      if obj['oid'] == oid:
        ...
        assert dist >= abs(self.agent_y - y) + abs(self.agent_x - x)
    ...
    success = mindist >= 0
    if success:
      #print(f'[{self._rank}]Move ({self.agent_x}, {self.agent_y}) -> ({obj_x}, {obj_y})')
      self.agent_x = obj_x
      self.agent_y = obj_y
    return mindist, success
  ...
  @property
  def parameters(self):
    """Parameters used to determine the map configuration."""
    return dict(
        init_object_list=self._init_object_list,
        init_item_map=self._init_item_map,
        init_obs=self._init_obs,
        init_pos_x=self.agent_init_pos_x,
        init_pos_y=self.agent_init_pos_y,
  ...
  def _validate_map(self):
    for obj_dict in self.object_list:
      oid = obj_dict['oid']
      item_id = oid + 3
      x, y = obj_dict['pos']
      if not self.obs[item_id][x][y] == 1:
        assert False, 'obs and obj_list are not matched'
      if not self.item_map[x][y] == item_id:
        assert False, 'item_map and obj_list are not matched'
  ...
  def _add_targets(self, empty_list):
    ...
    for obj_param in self.config.object_param_list:
      if 'max' in obj_param:
        oid = obj_param['oid']
        nb_obj = np.random.randint(0, obj_param['max'])
    ...
    self.agent_x = self.agent_init_pos_x
    self.agent_y = self.agent_init_pos_y
    #print(f'[{self._rank}]agent_x_y= ({ self.agent_x}, {self.agent_y})')
