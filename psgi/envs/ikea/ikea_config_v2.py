...
CONTINUE1_SUBTASKS = ['Click ContinueShipMethod']
...
CONTINUE3_SUBTASKS = ['Click ContinuePayment']
FINAL_SUBTASKS = [
    'Click Place Order',
    'Click EditShipping',
    'Click EditBilling',
...
SUBTASK_LIST = BASE_SUBTASKS + SELECT_SHIPMETHOD_SUBTASKS + \
    CONTINUE1_SUBTASKS + SHIPPING_SUBTASKS + CONTACT_SUBTASKS + \
    BILLING_SUBTASKS + CONTINUE2_SUBTASKS + PAYMENT_SUBTASKS + \
    CREDIT_SUBTASKS + GIFT_SUBTASKS + CONTINUE3_SUBTASKS + FINAL_SUBTASKS
...
class IKEA2(WobConfig):
  environment_id = 'ikea_v2'
  ...
  def _construct_task(self, seed: int, keep_pristine: bool = False):
    ...
    g.add_one_to_many(
        source='Click ContinueShipMethod',
  ...
  def get_previous_subtasks(self, completion: Dict[str, bool]):
    subtasks = []
    if completion['Click ContinueShipMethod']:
