...
class GRProp:
  ...
  def init_graph(self, graphs: Sequence[SubtaskGraph]):
    ...
    self._index_to_pool = np.full(shape=(batch_size, max_NP), fill_value=-1, dtype=np.int32)
    for bind, graph in enumerate(graphs):
      self.rew_tensor[bind, :len(graph.subtask_reward)] = graph.subtask_reward
      if isinstance(graph.index_to_pool, dict):
        for k, v in graph.index_to_pool.items():
          self._index_to_pool[bind, k] = v
      else:
        self._index_to_pool[bind, :len(graph.index_to_pool)] = graph.index_to_pool
