...
py::tuple expand_and_filter(
    ...
    long int V = bi_c_comp.shape[1];
    if (V != bi_e_comp.shape[1]) {
        throw std::invalid_argument("All comps must have same number of subtasks.");
    }
    long int Ve = bi_c_elig.shape[1];
    if (Ve != bi_e_elig.shape[1]) {
        throw std::invalid_argument("All eligs shape must be the same.");
    }
    if (V > 60) {
    ...
    for (int i = 0; i < C; ++ i) {
        ...
        const uint64_t elig_i_encoded = _encode_binary(&celig[i * Ve], Ve);
    ...
    for (int i = 0; i < E; ++ i) {
        ...
        const int8_t* elig_i = &eelig[i * Ve];   // array(V)
        ...
        auto is_compatible = [&](const std::vector<int8_t> comp_i_expanded) {
            ...
            for (int j = 0; j < Ve; ++ j) if (elig_i[j] != -1) {
        ...
        for (uint64_t s = 0; s < (1ULL << number_of_dontcares); ++ s) {
            ...
            if (is_compatible(comp_i_expanded)) {
                ...
                expanded_elig.emplace_back(elig_i, elig_i + Ve);
    ...
    auto result_elig = array_int8(std::vector<int> {(int) R, (int) Ve});
    int8_t* relig = (int8_t*) result_elig.request().ptr;
    for (int i = 0; i < R; ++ i) {
        for (int j = 0; j < Ve; ++ j) {
            relig[i * Ve + j] = expanded_elig[i][j];
        }
    }
